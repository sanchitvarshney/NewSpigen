...
const ActionMenu: React.FC<any> = ({ row }) => {
  ...
  const handlePrintInvoice = async (orderId: string, printInvType: string) => {
    dispatch(
      printSellInvoice({ so_invoice: orderId, printInvType: printInvType })
    );
  };
  ...
  const menuItems = [
    ...
    },
    {
      ...
      label: (
        <span
          ...
          style={{
            color:
              row?.isEwayBill === "Y" || row?.isEInvoice === "Y"
                ? "gray"
                : "inherit",
          }}
        >
    ...
    {
      key: "createInvoice",
      label: <span onClick={() => handleViewDebitNote(row)}>Debit Note</span>,
    },
    {
      key: "print",
      label: (
